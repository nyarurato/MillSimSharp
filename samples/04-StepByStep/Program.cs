...
Console.WriteLine("=== MillSimSharp Sample: Step-by-Step Execution (SDF) ===\n");
...
Console.WriteLine("Creating SDF grid (60×60×40mm, 1mm resolution)...");
...
while (executor.CurrentCommandIndex < executor.TotalCommands)
{
    ...
    // Safety check to prevent infinite loop if we're done but the loop condition is still true
    // (CurrentCommandIndex stops at Count-1, so checking < Count can be true even if done)
    if (executed == 0) break;
    ...
    if (executed > 0)
    {
        ...
        // Convert to mesh for export - this might be slow loop but demonstrates functionality
        var stepMesh = MeshConverter.ConvertToMeshFromSDF(sdfGrid);
        StlExporter.Export(stepMesh, stepOutputPath);
        Console.WriteLine($"    Saved: step_{stepCount:D2}.stl (using SDF mesh generation)\n");
    }
}
...
var finalMesh = MeshConverter.ConvertToMeshFromSDF(sdfGrid);
StlExporter.Export(finalMesh, finalOutputPath);
